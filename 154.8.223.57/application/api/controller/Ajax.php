...
// 从数据库查找城市的天⽓气信息
$good=db('Weather')->where('city', $city1)->find();
if(!empty($good)){
...
//$value = array("status"=>"1","msg"=>"保存成功,经度：$latitude纬度：$longitude ,城市详细信息 $contentStr, 城市：$city1 ,hgf $weather ,df$max,ddf $wind,dfff $min");
$value =
array("status"=>"1","city"=>"$city","weather"=>"$weather","wind"=>"$wind","
max"=>"$max","min"=>"$min","district"=>$district);
echo json_encode($value);
}
else {
...
$json=file_get_contents("http://api.map.baidu.com/geocoder/v2/?location=".$latitude.",".$longitude."&output=json&pois=1&ak=MqeRP24CZZCN4GSIpjAuQyTTDtiWs40Y");
